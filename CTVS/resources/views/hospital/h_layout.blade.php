...
<body class="crm_body_bg">
    ...
    <section class="main_content dashboard_part">
        ...
        @if (session()->has('hospital_name'))
        <h4 class="pe-5">Welcome : {{session()->get('hospital_name')}}</h4>

        <a href="{{url('/hospital/profile')}}">
            <img class="rounded-circle" src="/Hospital_Images/{{session()->get('hospital_image')}}" width="60px" alt="logo">
        </a>
    ...
    <nav class="sidebar">
        ...
        <ul id="sidebar_menu">
            ...
            <li class="mm-active">
                ...
                @if (session()->has('hospital_name'))
                <ul>
                    <li><a class="" href="{{ url('/hospital') }}">Home</a></li>
                </ul>
                <ul>
                    <li><a class="" href="{{ url('/hospital/add_vaccine') }}">Add Vaccine</a></li>
                </ul>
                <ul>
                    <li><a class="" href="{{ url('/hospital/all_vaccine') }}">All Vaccine</a></li>
                </ul>
                <ul>
                    <li><a class="" href="{{ url('/hospital/patient_result') }}">Covid-19 Result</a></li>
                </ul>
                <ul>
                    <li><a class="" href="{{ url('/hospital/vaccination_status') }}">Vaccination Status</a></li>
                </ul>
                <ul>
                    @if (session()->has('hospital_name'))
                    <li><a class="text-danger" href="{{ url('/hospital/h_logout') }}">Log Out</a></li>
                    @endif
                </ul>
                @endif
